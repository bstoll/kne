// Copyright 2022 Arista Networks
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//      http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.34.2
// 	protoc        v3.21.12
// source: ceos.proto

package ceos

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// cEOS-lab specific vendor data for KNE
type CEosLabConfig struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	ToggleOverrides map[string]bool `protobuf:"bytes,1,rep,name=toggle_overrides,json=toggleOverrides,proto3" json:"toggle_overrides,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"varint,2,opt,name=value,proto3"`
	WaitForAgents   []string        `protobuf:"bytes,2,rep,name=wait_for_agents,json=waitForAgents,proto3" json:"wait_for_agents,omitempty"`
}

func (x *CEosLabConfig) Reset() {
	*x = CEosLabConfig{}
	if protoimpl.UnsafeEnabled {
		mi := &file_ceos_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CEosLabConfig) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CEosLabConfig) ProtoMessage() {}

func (x *CEosLabConfig) ProtoReflect() protoreflect.Message {
	mi := &file_ceos_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CEosLabConfig.ProtoReflect.Descriptor instead.
func (*CEosLabConfig) Descriptor() ([]byte, []int) {
	return file_ceos_proto_rawDescGZIP(), []int{0}
}

func (x *CEosLabConfig) GetToggleOverrides() map[string]bool {
	if x != nil {
		return x.ToggleOverrides
	}
	return nil
}

func (x *CEosLabConfig) GetWaitForAgents() []string {
	if x != nil {
		return x.WaitForAgents
	}
	return nil
}

var File_ceos_proto protoreflect.FileDescriptor

var file_ceos_proto_rawDesc = []byte{
	,
}

var (
	file_ceos_proto_rawDescOnce sync.Once
	file_ceos_proto_rawDescData = file_ceos_proto_rawDesc
)

func file_ceos_proto_rawDescGZIP() []byte {
	file_ceos_proto_rawDescOnce.Do(func() {
		file_ceos_proto_rawDescData = protoimpl.X.CompressGZIP(file_ceos_proto_rawDescData)
	})
	return file_ceos_proto_rawDescData
}

var file_ceos_proto_msgTypes = make([]protoimpl.MessageInfo, 2)
var file_ceos_proto_goTypes = []any{
	(*CEosLabConfig)(nil), // 0: ceos.CEosLabConfig
	nil,                   // 1: ceos.CEosLabConfig.ToggleOverridesEntry
}
var file_ceos_proto_depIdxs = []int32{
	1, // 0: ceos.CEosLabConfig.toggle_overrides:type_name -> ceos.CEosLabConfig.ToggleOverridesEntry
	1, // [1:1] is the sub-list for method output_type
	1, // [1:1] is the sub-list for method input_type
	1, // [1:1] is the sub-list for extension type_name
	1, // [1:1] is the sub-list for extension extendee
	0, // [0:1] is the sub-list for field type_name
}

func init() { file_ceos_proto_init() }
func file_ceos_proto_init() {
	if File_ceos_proto != nil {
		return
	}
	if !protoimpl.UnsafeEnabled {
		file_ceos_proto_msgTypes[0].Exporter = func(v any, i int) any {
			switch v := v.(*CEosLabConfig); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_ceos_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   2,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_ceos_proto_goTypes,
		DependencyIndexes: file_ceos_proto_depIdxs,
		MessageInfos:      file_ceos_proto_msgTypes,
	}.Build()
	File_ceos_proto = out.File
	file_ceos_proto_rawDesc = nil
	file_ceos_proto_goTypes = nil
	file_ceos_proto_depIdxs = nil
}

// CI test modification
